...
<body>
    ...
    <div class="travel-content">
        <div class="travel-content-main">
            <h1>Sửa tin sự kiện</h1>
            <br />
            <form class="form" action="edit_event_exec.php" method="post" enctype="multipart/form-data">
                <input type="hidden" name="id" value="<?php echo $result["id"]?>" />

                <div class="clear-both"></div>
                <label>Image</label>
                <input name="img" type="file" value="<?php echo $result["image"]?>" />
                <img src="<?php echo $result["image"]?>" width="200px" />
                <div class="clear-both"></div>
                <label>Tên sự kiện</label><input name="title" value="<?php echo $result["name"]?>" />
                <div class="clear-both"></div>
                <label>Đơn vị tổ chức</label><input name="host" value="<?php echo $result["host"]?>" />
                <div class="clear-both"></div>
                <label>Ngày diễn ra hiện tại</label><input name="date"
                    value="<?php echo date('d/m/Y - H:i:s',strtotime($result["date"]));?>" />
                <div class="clear-both"></div>
                <label>Ngày thay đổi</label><input type="datetime-local" name="date" value="" />
                <div class="clear-both"></div>
                <label>Nội dung sự kiện</label><input name="content" value="<?php echo $result["content"]?>" />
                <div class="clear-both"></div>
                <label>Địa điểm</label><input name="address" value="<?php echo $result["address"]?>" />
